    ...
    <div v-if="step===1" class="options">
      <el-form ref="dataForm" :rules="rules" :model="compareParams.params" label-position="left" label-width="150px">
        ...
        <el-form-item label="账务类型">
          <el-select v-model="compareParams.params.billtype" placeholder="请选择">
            <el-option v-for="item in accountsTypeOptions" :key="item.code" :label="item.name" :value="item.code"/>
          </el-select>
        </el-form-item>
        <el-form-item label="账务条目">
          <el-select v-model="compareParams.params.billpid " placeholder="请选择">
            <el-option v-for="item in accountsCategoryOptions" :key="item.code" :label="item.name" :value="item.code"/>
          </el-select>
        </el-form-item>
      </el-form>
    </div>
    <div v-if="step===2" style="margin-top: 30px">
      ...
      <el-table
        ...
        <el-table-column
          ...
          :label="item.colcomments"
          :key="item.id"
          :fixed="item.colcomments === '订单消费金额' || item.colcomments === '订单结算金额' || item.colcomments === '银行实收金额' ? 'left' : null"
        />
      </el-table>
...
import { getDetailsCompareList } from '@/api/reconciliation'
...
export default {
  ...
  data() {
    return {
      ...
      dicParam: 'code_accountstype,code_accountscategory',
      uploadParams: {
  ...
  methods: {
    // 获取字典表
    getDic() {
      getDicMore(this.dicParam).then(response => {
        this.accountsTypeOptions = response.data.code_accountstype
        this.accountsCategoryOptions = response.data.code_accountscategory
      })
    },
    getCompareList(tab) {
      if (tab) {
      ...
      getDetailsCompareList(this.compareParams).then(response => {
    ...
    exportData(command) {
      if (this.compareData.preview.length === 0) {
        ...
        const url = process.env.BASE_API + 'file/exportBillDeatils'
