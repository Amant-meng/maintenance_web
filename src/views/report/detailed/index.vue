<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input v-model="listQuery.params.or_ci_tel" placeholder="联系电话" style="width: 150px;" class="filter-item"/>
      ...
      <el-select v-model="listQuery.params.payWay" placeholder="支付方式" clearable style="width: 200px" class="filter-item">
        <el-option v-for="item in enableOptions" :key="item.key" :label="item.label" :value="item.key"/>
      </el-select>
      <el-select v-model="listQuery.params.pay_trans_typebs" placeholder="交易类型" clearable style="width: 200px" class="filter-item">
        <el-option v-for="item in payTypeData" :key="item.code" :label="item.name" :value="item.code"/>
      </el-select>
    ...
    <div id="printTest" class="no-print">
      <el-table
        ...
        style="width: 100%;">
        <el-table-column label="预订单号" align="center" min-width="80">
          <template slot-scope="scope">
            <span>{{ scope.row.or_id }}</span>
          </template>
        </el-table-column>
        <el-table-column label="入住单号" min-width="80px" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column label="类型" align="center" min-width="80">
          <template slot-scope="scope">
            <span>{{ scope.row.or_modename }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="房间号" align="center" min-width="80">
          <template slot-scope="scope">
            <span>{{ scope.row.ro_id }}</span>
          </template>
        </el-table-column>
        <el-table-column label="房型" align="center" min-width="80">
          <template slot-scope="scope">
        ...
        <el-table-column label="入住时间" align="center" min-width="180">
          <template slot-scope="scope">
            <span>{{ scope.row.or_act_arr_time | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</span>
          </template>
        </el-table-column>
        <el-table-column label="退房时间" align="center" min-width="180">
          <template slot-scope="scope">
        ...
        <el-table-column label="支付金额" class-name="status-col" align="center" min-width="80">
          <template slot-scope="scope">
            <span>{{ scope.row.bill_all_pay_price }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="预付方式" class-name="status-col" align="center" min-width="80">
          <template slot-scope="scope">
            <span>{{ scope.row.pay_paytype }}</span>
          </template>
        </el-table-column>
        <el-table-column label="预付交易类型" class-name="status-col" align="center" min-width="80">
          <template slot-scope="scope">
            <span>{{ scope.row.pay_trans_typebs }}</span>
          </template>
        </el-table-column>
        <el-table-column label="交易编号" class-name="status-col" align="center" min-width="150">
          <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    getList() {
      if (this.listQuery.params.or_act_arr_time) {
        getSummarybillDetails(this.listQuery).then(response => {
          ...
          this.tatalall_price = response.attachData.bill_all_pay_money
    ...
    // 担保方式表格数据显示
    assureOptions(row, column) {
      for (let i = 0; i < this.assureParamsData.length; i++) {
        if (this.assureParamsData[i].code === row.pay_assure_id) {
          return this.assureParamsData[i].name
        }
      }
    },
    // 入住类型表格数据显示
    enterOptions(row, column) {
      for (let i = 0; i < this.enterParamsData.length; i++) {
        if (this.enterParamsData[i].code === row.or_mode) {
          return this.enterParamsData[i].name
        }
      }
    },
    // 预售支付方式表格数据显示
    payOptions(row, column) {
      for (let i = 0; i < this.payParamsData.length; i++) {
        if (this.payParamsData[i].code === row.pay_paytype) {
          return this.payParamsData[i].name
        }
      }
    },
